...
def configure(conf):
    ...
    # add -lzmq to compile command
    conf.env['lzmq'] = conf.check(mandatory=True, lib='zmq', uselib_store='LZMQ')
...
def create_ns3_program(bld, name, dependencies=('core',)):
    ...
    if program.env['ENABLE_STATIC_NS3']:
        if sys.platform == 'darwin':
            program.env.STLIB_MARKER = '-Wl,-all_load,-lzmq'
        else:
            program.env.STLIB_MARKER = '-Wl,-Bstatic,--whole-archive,-lzmq'
            program.env.SHLIB_MARKER = '-Wl,-Bdynamic,--no-whole-archive,-lzmq'
    else:
        if program.env.DEST_BINFMT == 'elf':
            ...
            if 'gcc' in (program.env.CXX_NAME, program.env.CC_NAME): 
                program.env.append_value ('SHLIB_MARKER', '-Wl,--no-as-needed,-lzmq')
