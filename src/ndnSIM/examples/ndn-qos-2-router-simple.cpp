...
namespace ns3 {
    ...
    main(int argc, char* argv[])
    {
       ...
       topologyReader.SetFileName("src/ndnSIM/examples/topologies/topology-2-src-1-dst.txt");
       topologyReader.Read();*/
       NodeContainer nodes;
       nodes.Create(5);
       ...
       // Middle link uses 1Mbps, the others use 10
       ...
       ndn::StrategyChoiceHelper::Install(nodes.Get(2), "/prefix", "/localhost/nfd/strategy/qos");
       ...
       apps = tokenHelper.Install(nodes.Get(2));
       apps = tokenHelper.Install(nodes.Get(3));

       // Producer
       // There will be one producers all for serving /prefix requests
